...
ID="floorp"; Path="$HOME/.local/share/floorp"; Exec="$Path/$ID"; Binary="$HOME/.local/bin/$ID"; Tarball=$(mktemp /tmp/$ID.XXXXXX.tar.xz)
Version=$(curl -s https://github.com/Floorp-Projects/Floorp/releases | grep -m 1 -A1 'ml-1 wb-break-all' | grep -o 'v.*' | sed 's/v//')
...
printf "\n[*] Extracting tarball\n"; tar xfv $Tarball > /dev/null 2>&1; mv $ID $Path 2>/dev/null; rm $Tarball
